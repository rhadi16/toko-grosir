<?php 
	...
	if ($action == "insert") {
		...
		for ($i=0; $i < $jum_data; $i++) {
			...
			$result = mysqli_query($mysqli, "INSERT INTO penjualan (id, id_barang, jum_yg_dibeli, tot_yg_dibeli, tanggal) 
											 VALUES(null, '$id_brg[$i]', '$jum_yg_dibeli[$i]', '$tot_yg_dibeli[$i]', '$tanggal[$i]')") or die(mysqli_error($mysqli));
