<?php 
	...
	if ($action == "insert") {
		...
		for ($i=0; $i < $jum_data; $i++) {
			...
			$result = mysqli_query($mysqli, "INSERT INTO pembelian (id, id_barang, harga_yg_dibeli, stok_yg_dibeli, stok_awal, tot_stok) 
											 VALUES(null, '$id_brg[$i]', '$harga_yg_dibeli[$i]', '$stok_yg_dibeli[$i]', '$stok_awal[$i]', '$tot_stok[$i]')") or die(mysqli_error($mysqli));
